...
public class OrderServiceImpl implements OrderService {
	...
	public void add(TbOrder order) {
		//获取购物车信息 订单模块一定登录过 redis一定能取得购物车
		List<Cart> stringUtil = (List<Cart>)redisTemplate.boundHashOps("cartList").get(order.getUserId());
		//支付日志
		List<String> orderIdList=new ArrayList();//订单ID列表
		double total_money=0;//总金额 （元）

		if(stringUtil != null){
			for (Cart cart : stringUtil) {

				//生成雪花片法id
				IdWorker idWorker = new IdWorker();
				long orderId = idWorker.nextId();
				System.out.println("cart idWorkerID : " + idWorker.nextId());
				...
				for (TbOrderItem orderItem : cart.getOrderItemList()) {
					...
					price += orderItem.getTotalFee().doubleValue();// 金额累加
					orderItemMapper.insert(orderItem);
				}
				//封装的数据
				tborder.setPayment(new BigDecimal(price));
